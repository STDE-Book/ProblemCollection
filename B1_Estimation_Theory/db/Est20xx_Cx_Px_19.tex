...
% [SUPRIMIDO: NO TIENE SOLUCIÓN ANALITICA]
% 	\part  Supuesto que se restringe la forma de los estimadores a cuadr\'{a}tico en $X$, determine el estimador $\hat{S}_{q,\text{MAD}}=w_{2}X^{2}$ que minimiza el coste absoluto medios.
	\end{parts}
...
$$
\hat{S}_\text{MMSE}(X)=\hat{S}_\text{MAD}(X) = |X|/2
$$
...
\end{align*}
Por tanto
$$\hat{S}_{q,\text{MMSE}}(X)=3X^2/5$$

%\part
%
%The MAD for any estimator in the form $w_2X^2$ is given by
%\begin{align*}
...
%\end{align*}
%
%It is easy to check that the minimum must be at some point in $0 \le w_2 \le 1$, but it has no analytical expression.
...
\part Find the estimators of $S$ based on variable $X$ that minimize the mean square error (MSE), ($\overline{C}_\text{MSE}= \mathbb E\left\{(S-\hat{S})^2 \right\}$) and mean absolute deviation (MAD) ($\overline{C}_\text{MAD}= \mathbb E\left\{ |S-\hat{S}| \right\}$), $\hat{S}_\text{MMSE}$ and $\hat{S}_\text{MAD}$, respectively.

\part If the estimator is constrained to be quadratic in the observations, obtain the expressions of the optimal estimators with respect to cost MSE, i.e., $\hat{S}_{q,\text{MMSE}}=w_1 X^2$. 

% [REMOVED: IT HAS NO ANALYTICAL SOLUTION]
% \part If the estimator is constrained to be quadratic in the observations, obtain the expressions of the optimal estimators with respect to MAD, i.e., $\hat{S}_{q,\text{MAD}}=w_2X^2$, respectively. 
\end{parts}
