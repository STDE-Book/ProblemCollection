...
Determine:
\begin{parts}
\part el estimador bayesiano, $\hat{S}_C$ para el coste dado.
\part el estimador lineal $\hat{S}_L=wX$ que minimiza el riesgo $\EE\left\{ c(S,\hat{S}) \right\}$.
\part el riesgo de ambos estimadores, $\EE\left\{ c(S,\hat{S}_C) \right\}$  y $\EE\left\{ c(S,\hat{S}_L) \right\}$ .
\part el sesgo no condicionado de ambos estimadores.
...
\part El sesgo es
\begin{align*}
B_C =\EE\left\{ \hat{S}_C - S\right\} 
	&=  \frac{4}{5} \EE\left\{X^2 \right\} - \EE\{S\}
	 =  \frac{4}{7} - \frac{10}{21} = \frac{2}{21}  \\
B_L =\EE\left\{ \hat{S}_L - S\right\} 
	&= \frac{11}{15} \EE\left\{X \right\} - \EE\{S\}
	 = \frac{11}{18} - \frac{10}{21} = \frac{17}{126}  
\end{align*}
...
Find:
\begin{parts}
\part The Bayesian estimator, $\hat{S}_C$, for the given cost.
\part The linear estimator $\hat{S}_L=wX$ which minimizes the risk $\EE\left\{ c(S,\hat{S}) \right\}$.
\part The risk of both estimators: $\EE\left\{ c(S,\hat{S}_C) \right\}$ and $\EE\left\{ c(S,\hat{S}_L) \right\}$.
\part The unconditional bias of both estimators.
\part The variance of the error both estimators: $\text{var}\left\{ S-\hat{S}_C \right\}$ and $\text{var}\left\{ S-\hat{S}_L \right\}$.
\end{parts}
...
\part Using the bias-variance decomposition,
\begin{align*}
\text{Var}\left\{ S-\hat{S}_C \right\} 
	&= \EE\left\{\left(S-\hat{S}_C \right)^2\right\} - B_C^2   \\
	&= \EE\left\{S^2\right\} + \EE\left\{\hat{S}_C^2\right\} 
