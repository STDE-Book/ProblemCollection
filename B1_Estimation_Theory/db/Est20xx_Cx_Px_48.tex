...
\begin{parts}
\part Obtenga el valor medio de $X_3$, $\EE[X_3]$, su varianza, $v_{X_3}$, y su covarianza con $S$, $v_{S,X_3}$.
\part Obtenga el estimador de MMSE de $S$ empleando solo $X_3$, $\hat S_{\text{MSE},3}$.
\part Sabiendo que $\EE\{S^2\}=1$, obtenga el error cuadrático medio del estimador $\hat S_{\text{MSE},3}$.
\end{parts}

\begin{solution}
\begin{parts}
\part $\EE\{X_3\}=0$,$v_{X_3}=3$, $v_{S,X_3}=3/2$.
\part $\hat S_{\text{MSE},3}= 1/2 X_3$.
\part $\EE\{(S-\hat S_{\text{MSE},3})^2\}=1/4$.
\end{parts}
\end{solution}
...
\begin{parts}
\part Compute the mean value of $X_3$, $\EE\{X_3\}$, its variance, $v_{X_3}$, and its covariance with $S$, $v_{S,X_3}$.
\part Compute the minimum MSE estimator of $S$ based on $X_3$ only, $\hat S_{\text{MSE},3}$.
\part Given that $\EE\{S^2\}=1$, compute the MSE of $\hat S_{\text{MSE},3}$.
\end{parts}
