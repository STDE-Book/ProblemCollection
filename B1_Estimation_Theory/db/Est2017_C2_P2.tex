...
Also, we know that $S$ es una Gaussian random variable with mena $4$ y and variance $0.9$.
\begin{parts}
	\part Compute the MMSE estimator of $S$ given $X_1$. Denote it as $\hat{S}_{1}$.
	\part Compute the Mean Square Error for estimator $\hat{S}_1$.
	\part Compute the MMSE estimator of $S$ given $Z = \frac{1}{2}(X_1 + X_2)$. Denote it as $\hat{S}_{z}$.
