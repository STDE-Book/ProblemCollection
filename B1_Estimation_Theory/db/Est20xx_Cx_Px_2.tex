...
\question Consider the estimation of a random variable $S$ from another random variable $X$, given the joint probability density function (pdf)
$$
p_{S,X}(s,x) =\frac{6}{7} \left(x+s\right)^2, \qquad 0 \le  x \le 1, \quad  0 \le s \leq 1
$$
...
\part Compute the minimum MSE estimator of $S$ given $X$, $\SMSE$.
\part Compute the MAP estimator of $S$ given $X$, $\SMAP$.
\part Compute the bias and the variance of the MAP estimator.
\end{parts}
...
\begin{align*}
\EE\{\SMAP\} - \EE\{S\} = 1 - \frac{9}{14} = \frac{5}{14}
\end{align*}
Since $\SMAP=1$ (constant and independent of $X$), its variance is zero.
\end{parts}
\end{solution}
\fi