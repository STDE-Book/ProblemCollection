...
\question Se dispone de $K$ muestras, $\left\{x_k\right\}^{K-1}_{k=0}$, tomadas independientemente, de una v.a. X cuya d.d.p. viene dada por 
 	$$p_X(x)=\frac{1}{bx^2} \exp\left(-\frac{1}{bx} \right),  \qquad  x\ge 0 $$
con $b>0$.
\begin{parts}
...
\begin{align*}
\hat{b}_\text{ML}
	&= \argmax_b \sum_{k=0}^{K-1} \log\left(p_X(x_k)\right)  \\
	&= \argmax_b \sum_{k=0}^{K-1} \log\left(
						\frac{1}{b x_k^2} \exp\left(-\frac{1}{bx_k} \right)
						\right)  \\
	&= \argmax_b \left( - K \log(b) - 2 \sum_{k=0}^{K-1} \log\left(x_k\right)
	                    - \frac1b \sum_{k=0}^{K-1} \frac{1}{x_k} \right)  \\
	&= \argmax_b \left( - K \log(b) - \frac1b \sum_{k=0}^{K-1} \frac{1}{x_k} \right)  \\
    &= \frac{1}{K} \sum_{k=0}^{K-1}{\frac{1}{x_k}}
\end{align*}
...
Dado que 
\begin{align*}
\hat{B}_\text{ML} &= \frac{1}{K} \sum_{k=0}^{K-1}{\frac{1}{X_k}}
\end{align*}
la media del estimador es
\begin{align*}
\EE\{\hat{B}_\text{ML}\} &= \frac{1}{K} \sum_{k=0}^{K-1} \EE\left\{\frac{1}{X_k}\right\}  
...
\question We have access to a set of $K$ samples, $\left\{X_k\right\}^{K-1}_{k=0}$, independently drawn from a random variable $X$ with p.d.f. 
 	$$p_X(x)=\frac{1}{bx^2} \exp\left(-\frac{1}{bx} \right),  \qquad  x\ge 0 $$
with $b>0$ a constant.
\begin{parts}
