...
\part Calcule el sesgo no condicionado de los estimadores anteriores. 
\end{parts}
...
\begin{parts}
\part $\SMSE = \frac{2}{3}\left(X-1 \right) $
\part $\SMAP = X-1 $
\part $\SMAD = \frac{1}{\sqrt{2}}\left(X-1 \right)  $
\part $\EE\left\{\SMSE-S \right\} = 0    \quad
       \EE\left\{\SMAP-S  \right\} = 0.25 \quad
       \EE\left\{\SMAD-S  \right\} = 0.03 $
...
$$p_{S,X}(s,x)= \left\{ \begin{array}{ll} 6s, & 0<s<x-1, \; \; 1<x<2 \\
0, & \mbox{otherwise}  \end{array}  \right. $$

Obtain:
\begin{parts}
\part The minimum MSE estimator of $S$ given $X$, $\SMSE$.
\part The maximum {\em a posteriori} estimator of $S$ given $X$, $\SMAP$.
\part The minimum mean absolute error estimator of $S$ given $X$, $\SMAD$.
\part The unconditional bias of all previous estimators. 
\end{parts}
