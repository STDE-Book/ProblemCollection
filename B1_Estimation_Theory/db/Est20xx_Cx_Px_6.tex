...
\begin{parts}
\part Determínese el estimador de máxima verosimilitud de $a$, $\hat{A}_\text{ML}$, basado en un conjunto de $K$ observaciones independientes de la variable aleatoria X, $\left\lbrace X^{(k)} \right\rbrace_{k=1}^{K} $ .
...
\begin{parts}
\part $\hat{A}_\text{ML} = \dfrac{1}{K} \sum_{k=1}^{K} {X^{(k)}}$
\part $\hat{A} = \frac{c}{K} \sum_{k=1}^{K} {X^{(k)}}$ \\
 $\mathbb E\left\lbrace \hat{A} - a \right\rbrace^2 = \left(c-1 \right)^2 a^2$, \hspace{.5cm}
 $\text{Var} \left\lbrace \hat{A} \right\rbrace = \dfrac{c^2 a^2}{K}$, \hspace{.5cm}
 $\mathbb E\left\lbrace \left( \hat{A}-a\right) ^2 \right\rbrace = \left(c-1 \right)^2 a^2+ \displaystyle\frac{c^2 a^2}{K}$
\part $c^* = \dfrac{K}{K+1}$, \hspace{0.5cm} 
$c^*\longrightarrow 1 \; (K\longrightarrow \infty)$, \hspace{0.5cm} 
$\mathbb E\left\lbrace \left( \hat{A}-a\right) ^2 \right\rbrace = \dfrac{a^2}{K+1} \; (c=c^*)$
...
\begin{parts}
\part Obtain the maximum likelihood estimator of $a$, $\hat{A}_\text{ML}$, based on a set of $K$ independent observations of random variable $X$, $\left\lbrace X^{(k)} \right\rbrace_{k=1}^{K} $.
...
\begin{solution}
\begin{parts}
\part  $\hat{A}_\text{ML}=\displaystyle\frac{1}{K} \sum_{k=1}^{K} {X^{(k)}}$
\part $\hat{A}=\displaystyle\frac{c}{K} \sum_{k=1}^{K} {X^{(k)}}$ \\
  $\mathbb E\left\lbrace \hat{A}-a \right\rbrace^2=\left(c-1 \right)^2 a^2$, \hspace{.5cm}
 $\text{Var} \left\lbrace \hat{A} \right\rbrace=\displaystyle\frac{c^2 a^2}{K}$, \hspace{.5cm}
 $\mathbb E\left\lbrace \left( \hat{A}-a\right) ^2 \right\rbrace= \left(c-1 \right)^2 a^2+ \displaystyle\frac{c^2 a^2}{K}$
\part $c^*=\displaystyle\frac{K}{K+1}$, \hspace{0.5cm} $c^*\longrightarrow 1 \; (K\longrightarrow \infty)$, \hspace{0.5cm} $\mathbb E\left\lbrace \left( \hat{A}-a\right) ^2 \right\rbrace=\displaystyle\frac{a^2}{K+1} \; (c=c^*)$
\part The range of values is: $c \in \left[ \displaystyle\frac{K-1}{K+1}, 1 \right]$, which narrows as $K$ increases.
  \end{parts}
  \end{solution}
