...
\begin{parts}
\part Obtain the ML decider based on the set of observations $\left\lbrace X^{(k)} \right\rbrace_{k=1}^K$ and check, using such a classifier, that $T=\sum_{k=1}^K X^{(k)}$ is a sufficient statistic for the decision.
\end{parts}
...
\part Calculate $P_{\rm FA}$ and $P_{\rm M}$ for the following threshold decider, as a function of $\eta$:
$$ t \dceroduno \eta$$
\part Provide an approximate plot of the ROC curve for the previous decider, indicating:
\begin{itemize}
