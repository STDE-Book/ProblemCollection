...
$$t \dunodcero \eta,$$
where we have defined random variable $T = X_1 + X_2$. Obtain the threshold value corresponding to the ML classifier.
...
\part Sketch in an approximate manner the OC curve, and place on this curve the operation points corresponding to $\eta=0$, $\eta= \infty$, the Neyman-Pearson detector with false alarm probability $P_\text{FA} = 0.1$, and the ML test for the particular case $b=3a$.
\part If both hypotheses have the same {\em a priori} probability, calculate the average risk of the decision maker for the following cost policy: $c_{00}=0$, $c_{11} = 0.5$, and $c_{01} = c_{10} = 1$. Obtain the threshold value that minimizes this average risk.
\end{parts}
