...
p_{X_n|H}(x|0) &= 2 \exp(- 2 x) u(x)   \qquad  \qquad n=1,2,3 \nonumber 
\end{align}
Three MAP decision makers are applied, one for each variable, in such a way that decision $D_n$ of the $n$-th decision maker is based in observation $X_n$ only (for $n=1$, 2 or 3). 
...
\part Let $Z = (D_1,D_2,D_3)$ the vector containing the three decisions. Consider the MAP decision maker based on observation ${\bf Z}$ (that is, the decision maker does not observe $X_1$, $X_2$ or $X_3$, and its only input is ${\bf Z}$). Determine its decision when ${\bf Z} = (1,1,0)$.
\end{parts}
